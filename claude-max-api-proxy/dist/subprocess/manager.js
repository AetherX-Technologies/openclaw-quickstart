...
export class ClaudeSubprocess extends EventEmitter {
    ...
    buildArgs(prompt, options) {
        const args = [
            ...
            "--dangerously-skip-permissions", // Allow full tool/bash access
    ...
    processBuffer() {
        ...
        for (const line of lines) {
            ...
            try {
                ...
                console.error(`[DEBUG] msg type=${message.type} subtype=${message.subtype} result=${JSON.stringify(message.result || '').slice(0,80)}`);
                ...
                if (isContentDelta(message)) {
                    ...
                    // Track last assistant text for fallback when result.result is empty
                    const textBlocks = message.message?.content?.filter(c => c.type === "text") || [];
                    if (textBlocks.length) this._lastAssistantText = textBlocks.map(c => c.text).join("");
                    this.emit("assistant", message);
                }
                else if (isResultMessage(message)) {
                    // If result.result is empty but we have accumulated assistant text, use that
                    if (!message.result && this._lastAssistantText) {
                        message.result = this._lastAssistantText;
                    }
